...
public class ListEditorOverlay<T> extends WindowOverlay {
    ...
    // TODO: i18n
    private final ActionButtonWidget addButton = new ActionButtonWidget(ComponentCompat.literal("+ Add"), (btn) -> addNewItem());
    private final ActionButtonWidget removeButton = new ActionButtonWidget(ComponentCompat.literal("- Remove"), (btn) -> removeSelectedItem());
    private final ActionButtonWidget moveUpButton = new ActionButtonWidget(ComponentCompat.literal("▲ Up"), (btn) -> moveSelected(-1));
    ...
    private void rebuildItemWidgets() {
        ...
        for (int i = 0; i < workingList.size(); i++) {
            final int index = i;
            Option<T> tempItemOption = new Option<>(
                ComponentCompat.literal("Item " + (i + 1)),
                "item." + i,
    ...
    private void addNewItem() {
        ...
        int insertIndex = (selectedIndex >= 0 && selectedIndex < workingList.size())
            ? selectedIndex + 1
            : workingList.size();
    ...
    private void moveSelected(int direction) {
        syncWorkingListFromWidgets();
        if (selectedIndex < 0 || selectedIndex >= workingList.size()) return;
    ...
    public void render(Font font, GuiCompat gui, LayoutEngine layout, int x, int y, int width, int height, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < itemWidgets.size(); i++) {
            ...
            String numText = "#" + (i + 1);
            gui.text(font, ComponentCompat.literal(numText), listX + 6, itemY + (itemHeight - 8) / 2, Colors.WHITE, false);
        ...
        okButton.renderWithBackground(font, gui, layout, okX, footerY, confirmBtnW, confirmBtnH, mouseX, mouseY, COLOR_BTN_OK_BG);
