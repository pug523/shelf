...
public class ConfigTreeWalker {
    ...
    public static boolean isLeafType(Class<?> type) {
        return type.isPrimitive() || type == Boolean.class || type == Integer.class || type == Float.class
                || type == Double.class || type == String.class || type.isEnum() || Item.class.isAssignableFrom(type)
    ...
    private record ContextImpl(Field field, Class<?> type, Object instance, Object defaultInstance, String keyPath,
            ConfigEntry entry, boolean isLeaf, Runnable recurseRunnable) implements FieldContext {
        @Override
        public void recurse() {
            recurseRunnable.run();
        }
