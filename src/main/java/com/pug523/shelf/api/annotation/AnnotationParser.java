...
import com.pug523.shelf.ui.option.ListOptionWidget;
...
import static com.pug523.shelf.common.compat.JavaCompat.listOf;
...
public class AnnotationParser {
    ...
    public static <T> ConfigScreen buildScreen(Component title, Screen parent, T configInstance, T defaultInstance,
            Runnable onSave, LayoutConfig layoutConfig) {
            ...
            ConfigTreeWalker.walk(configInstance, defaultInstance, ctx -> {
                ...
                if (hasCategory) {
                    ...
                    if (targetContext == null) {
                        ...
                        activeContext.getBuilder().subCategory(ComponentCompat.translatable(titleKey),
                                b -> holder[0] = new ParsingContext(b));
                ...
                if (hasCategory) {
                    String localKey = !entry.key().isEmpty() ? entry.key()
                            : ConfigTreeWalker.camelToSnake(ctx.field().getName());
                    ...
                    groupKey = (entry != null && !entry.group().isEmpty()) ? entry.group()
                            : "all_settings.default_group";
                ...
                if (ctx.isLeaf()) {
                    OptionWidget<?> widget = createWidgetForField(ctx.field(), ctx.instance(), ctx.defaultInstance(),
                            optionKey, descKey);
                    if (widget != null) {
                        ...
                        if (groupBuilder == null) {
                            ...
                            targetContext.getBuilder().group(ComponentCompat.translatable(groupKey),
                                    b -> holder[0] = b);
                    ...
                    if (hasCategory) {
                        ...
                        targetContext.getBuilder().subCategory(ComponentCompat.translatable(subCategoryTitleKey),
                                subBuilder -> {
    ...
    private static OptionWidget<?> createWidgetForField(Field field, Object instance, Object defaultInstance,
            String optKey, String descKey) {
        try {
    ...
    public static <T> OptionWidget<List<T>> createListWidgetForField(Field field, GuiOption<List<T>> option) {
        try {
            ...
            Function<GuiOption<T>, OptionWidget<T>> itemWidgetFactory = (tempOpt) -> WidgetRegistry
                    .createWidgetForType(elementType, finalChildAnno, tempOpt);

            Supplier<T> defaultValueSupplier = () -> {
                if (listAnno.creator() != WidgetTypes.List.DefaultCreator.class) {
                    try {
                        Supplier<T> customCreator = (Supplier<T>) listAnno.creator().getDeclaredConstructor()
                                .newInstance();
